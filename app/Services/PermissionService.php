...
class PermissionService
{
    public function saveSitePermission($site)
    {
        ...
        $modules->map(
            function ($module) use ($site) {
                collect($module['menus'])->map(
                    function ($menu) use ($site, $module) {
    ...
    public function checkModuleAccess(Site $site, string $name)
    {
        ...
        return (bool)collect($module['menus'])->filter(
            function ($menu) use ($site, $module) {
                return (bool)collect($menu['items'])->filter(
                    function ($item) use ($site, $module) {
                        $name = permission_name($item['permission'], $site, $module);
