                ...
                <div class="bg-white border shadow-sm p-4">
                    ...
                    <div class="d-flex justify-content-start flex-wrap mt-3 tags ">
                        <a href="{{route('edu.front.lesson.index')}}"
                           class="btn btn-outline-secondary btn-sm mr-3 mb-2">全部</a>
                        @foreach($tags as $tag)
                            <a href="{{route('edu.front.lesson.index',['tag'=>$tag->title])}}"
                               class="btn btn-sm mr-3 mb-2 {{ $tag->title==request()->query('tag')?'btn-secondary':'btn-outline-secondary' }}">
                                {{$tag['title']}}
                            </a>
                        @endforeach
                                ...
                                <a href="{{ route('edu.front.lesson.show',$lesson) }}" class="d-block shadow-sm lesson">
                                    <div class="card">
                                        <div class="card-body p-0"><img
                                                src="{{$lesson['thumb']}}"></div>
                                        <div class="card-body lesson-title">{{$lesson['title']}}</div>
                                        <div
                                            class="card-footer d-flex justify-content-between text-secondary small bg-light">
                                            <span>开始学习</span>
                                            <span>
